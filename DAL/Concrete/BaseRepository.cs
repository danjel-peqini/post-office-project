...
namespace DAL.Concrete
{
    internal class BaseRepository<TEntity, TKey> : IRepository<TEntity, TKey> where TEntity : class
    {
        ...
        // Get Paginated data using dynamic linq
        protected IQueryable<T> PaginationConfiguration<T>(IQueryable<T> result, string? sortField, string? sortOrder, string? searchValue) where T : class
        {
            if (!string.IsNullOrEmpty(searchValue))
            {
                //search value dynamic using reflection
                result = SearchHelper<T>.SearchForStringInPropertiesAsQueryable(result, searchValue);
            }
            if (!string.IsNullOrWhiteSpace(sortField))
            {
                //order by all columns using reflection
                if (sortOrder == "desc")
                {
                    result = SortList.OrderByDescendingPropertyAsQueryable(result, sortField);
                }
                else
                {
                    result = SortList.OrderByPropertyAsQueryable(result, sortField);
                }
            }
            return result;
        }
